...
public enum OperationType {
    ...
    //unary
    ACOS(OperationMainType.UNARY, "-?acos", "acos", 4, AcosCalcOperation.class),
    ASIN(OperationMainType.UNARY, "-?asin", "asin", 4, AsinCalcOpearion.class),
    ...
    public static OperationType[] getBinary(){
        return new OperationType[]{ADD, MINUS, MULTIPLY, DIVIDE, POWER};
    }

    public static boolean isBinary(String description) {
        OperationType[] binaryOperations = getBinary();
        for (OperationType operation : binaryOperations){
            for(String opDescription : operation.getDescriptions()){
                if (description.equals(opDescription)){
                    return true;
                }
            }
        }
    ...
    public static boolean isNumber(String description) {
        return description.matches(NUMBER.getRegEx());
    }
